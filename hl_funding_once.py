...
# Screener config
TOP_N = 5                  # How many coins to surface in each direction
MIN_OI_USD = 1_000_000     # Filter out illiquid coins (OI < $1M)
OI_SURGE_PCT = 0.05        # OI increased >5% since last reading = surge

# Coins you always want in the report regardless of signal
PINNED_COINS = ["BTC", "ETH", "HYPE"]

# Your spot buy targets (optional, for price alerts)
SPOT_TARGETS = {
    "BTC": 40000,
    "ETH": 1200,
    "HYPE": 40,
...
def fetch_hl_rates():
    """Fetch funding rates for every active perp on Hyperliquid."""
    try:
        res = requests.post(
            HL_API,
        ...
        for i, asset in enumerate(assets):
            ...
            oi_usd = oi * mark_px  # OI in USD terms
...
def load_oi_log():
    """Load previous OI readings from JSON log."""
...
def save_oi_log(oi_data):
    """Save current OI readings for next run comparison."""
    with open(OI_LOG_FILE, "w") as f:
        json.dump(oi_data, f)

def compute_oi_delta(coin, current_oi, prev_oi_log):
    """
    Returns (oi_delta_pct, oi_surge_flag).
    oi_delta_pct: % change in OI vs last reading
    oi_surge_flag: True if OI rose more than OI_SURGE_PCT
    """
...
def load_funding_momentum():
    """
    Read the last 3 funding readings per coin from funding_log.csv
    to assess whether funding is trending more negative.
    Returns dict: coin -> list of last N funding_8h values (oldest first)
    """
    ...
    # Keep only last 3 readings per coin
...
def is_funding_trending_negative(coin, current_funding_8h, momentum_history):
    """
    True if the last 3 readings (including current) are each more negative
    than the previous — i.e. funding is accelerating negative.
    """
    hist = momentum_history.get(coin, [])
    series = hist + [current_funding_8h]  # append current
    if len(series) < 3:
        return False  # not enough data yet
    # Check each step is lower than the previous
...
def screen_coins(hl_data, prev_oi_log, momentum_history):
    """
    Returns enriched data dict for all liquid coins,
    plus top_neg and top_pos lists for the screener report.
    """
    enriched = {}
...
def main():
    ...
    # 2. Screener report — top negative + top positive
    ...
    print(f"{'─'*60}")
    print(f"TOP {TOP_N} MOST NEGATIVE FUNDING:")
    for coin, d in top_neg:
        oi_tag = " [OI SURGE]" if d["oi_surge"] else ""
        print(f"  {coin:8s} APY: {d['funding_apy']:+.2%}  OI: ${d['oi_usd']/1e6:.1f}M{oi_tag}  {d['signal']}")

    print(f"\nTOP {TOP_N} MOST POSITIVE FUNDING:")
    for coin, d in top_pos:
        print(f"  {coin:8s} APY: {d['funding_apy']:+.2%}  OI: ${d['oi_usd']/1e6:.1f}M  {d['signal']}")

    if squeeze_setups:
        print(f"\n🔥 SQUEEZE SETUPS: {[c for c, _ in squeeze_setups]}")

    print(f"\nLogged {len(enriched)} coins to {LOG_FILE}")
